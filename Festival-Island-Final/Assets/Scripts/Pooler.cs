...
public class Pooler : MonoBehaviour
{
    ...
    //we use spawnPoints to store unused slicing cakes
    public List<GameObject> SlicingTargets;
     ...
     void FirstPooling()
    {
        ...
        //for regular fruits
        foreach (GameObject fruit in SlicingTargets)
        {
            for (int i = 0; i < 10; i++)
            {
    ...
    public void GetParticle(particleType _particleType, Vector3 pos, Quaternion rot)
    {
        ...
        // }
        if (_particleType == particleType.Explosion)
        {
            ResetPool();
            StartGameInPool(2f);
        }
